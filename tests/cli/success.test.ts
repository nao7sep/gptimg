...
describe("CLI success contracts", () => {
  ...
  it("reports usage errors for invalid local option parsers", async () => {
    const cases = [
      ...
      [
        "bad chroma float",
        ["chroma", "--in", "in.png", "--outer-threshold", "wide"],
        "--outer-threshold: not a number",
      ],
      [
        "bad inspect int",
        ["inspect", "--in", "in.png", "--edge-band-dilate", "large"],
        "--edge-band-dilate: not a number",
      ],
      [
        "bad inspect choice",
